...
uint8_t RecvdCommand[11];
uint8_t counterBytesCommand = 0;

comState_t comState = IDEL;
float Postion, DurationPosition, curPosition;
...
uint8_t ProcessReceivedMessage(uint8_t *message) {
	...
	if (message[0] == 72 && message[1] == 90) {
		...
		case SET_POSITION: {
			MC_StartMotor1();
			ProcessReceivedFloat(FirstBytesRevdMSG, &Postion);
			ProcessReceivedFloat(SecondBytesRevdMSG, &DurationPosition);
			comState = SET_POSITION;
			}
			break;
		case SET_SPEED: {
			MC_StartMotor1();
			ProcessReceivedUint16(FirstBytesRevdMSG, &Time);
			ProcessReceivedInt16(SecondBytesRevdMSG, &Speed);
			comState = SET_SPEED;
		}
			break;
		case SET_TORQUE: {
		...
		{
			// MC_StartMotor1();
			comState = GET_POSITION;
		}
		break;
		case GET_CONTROL_MODE: {
			// MC_StartMotor1();
			comState = GET_CONTROL_MODE;
		}
			break;
		default:
			break;
...
//      clearCommand();
		...
		case STOP_MOTOR:
			...
			break;
		case SET_POSITION:
			MC_ProgramPositionCommandMotor1(Postion, DurationPosition);
			break;
		case SET_SPEED:
			MC_ProgramSpeedRampMotor1(Speed, Time);
			break;
		case SET_TORQUE:
			MC_ProgramTorqueRampMotor1(Torque, Time);
			break;
		case GET_POSITION: {
			curPosition = MC_GetCurrentPosition1();
			ConvertFloatTwoBytes(curPosition, FirstBytesSentMSG);
			SendMessage(GET_POSITION, FirstBytesSentMSG, FirstBytesSentMSG);
			comState = IDEL;
		}
			break;
		case GET_CONTROL_MODE: {
			contMode = MC_GetControlModeMotor1();
			FirstBytesSentMSG[0] = contMode;
			SendMessage(GET_CONTROL_MODE, FirstBytesSentMSG, FirstBytesSentMSG);
			comState = IDEL;
		}
			break;
		}
...
//      if (fTargetPos == 0)
//      {
//        fTargetPos = ((float)(1*360) * (float)(M_PI)) / 180.0f;
//      }
//      else
//      {
//        fTargetPos = 0;
...
//    }
//    MC_ProgramSpeedRampMotor1(100, 2000);
		/* --------------------------------------------------- */
...
static void MX_USART1_UART_Init(void) {
	...
	LL_USART_TransmitData8(USART1, 72);
...
static void MX_GPIO_Init(void) {
	...
	/**/
	LL_GPIO_SetPinPull(M1_ENCODER_Z_GPIO_Port, M1_ENCODER_Z_Pin,
			LL_GPIO_PULL_NO);

	/**/
	LL_GPIO_SetPinMode(M1_ENCODER_Z_GPIO_Port, M1_ENCODER_Z_Pin,
			LL_GPIO_MODE_INPUT);
